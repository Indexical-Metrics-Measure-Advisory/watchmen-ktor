...
private fun exists(value: Any?, values: Any?): Boolean {
    return if (values is Iterable<*>) {
        ...
        throw throw RuntimeException("In operator is only compatible for right values are iterable, currently is [$values].")
    }
}
...
private fun notExists(value: Any?, values: Any?): Boolean {
    return if (values is Iterable<*>) {
        ...
        throw throw RuntimeException("Not-in operator is only compatible for right values are iterable, currently is [$values].")
    }
}
